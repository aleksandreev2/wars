...
- Server owns match state, scoring, arena mutation, and objective validation.
- A point is awarded only after the enemy objective bed has been destroyed and that defender is then killed by the opposing player.
...
- Arena reset must remove player-placed blocks and restore objective beds without rebuilding the whole castle during active play.
